<section class="mx-auto flex w-full max-w-7xl flex-col gap-6">
    <div><flux:heading size="xl" level="1">{{ __('Application review') }}</flux:heading><flux:text class="mt-2">{{ __('Review submitted applications. Verification records review completion, not an admission decision.') }}</flux:text></div>
    ...
    <div><flux:button size="sm" wire:click="clearFilters">{{ __('Reset filters') }}</flux:button></div>
    <flux:error name="search" /><flux:error name="roundFilter" /><flux:error name="statusFilter" />
            ...
            <flux:table.rows>
                @forelse ($records as $record)
                    <flux:table.row :key="$record->id">
                        <flux:table.cell><div class="font-medium">{{ $record->application_code }}</div><div>{{ $record->candidateProfile?->user?->name ?? __('Unavailable candidate') }}</div><div class="text-xs">{{ $record->candidateProfile?->candidate_code }} · {{ $record->candidateProfile?->user?->email }}</div></flux:table.cell>
                        ...
                        <flux:table.cell><div>{{ $record->reviewer?->name ?? __('Not recorded / unavailable') }}</div><div class="text-xs">{{ $record->reviewed_at?->format('Y-m-d H:i:s') }}</div></flux:table.cell>
                        <flux:table.cell><flux:button size="sm" :href="route('admin.applications.show', $record->id)" wire:navigate>{{ __('Open review') }}</flux:button></flux:table.cell>
                    </flux:table.row>
                @empty
                    <flux:table.row><flux:table.cell colspan="6"><div class="py-10 text-center">{{ __('No applications match this queue. Try changing or clearing the filters.') }}</div></flux:table.cell></flux:table.row>
                @endforelse
            </flux:table.rows>
        </flux:table>
    </div>
