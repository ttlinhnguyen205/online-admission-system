<section class="mx-auto flex w-full max-w-7xl flex-col gap-6">
    ...
    <div class="flex flex-wrap gap-3">
        ...
        @if ($application)
            <flux:button :href="route('candidate.applications.show', $application->id)" wire:navigate>{{ __('Quay lại phần hồ sơ và nguyện vọng') }}</flux:button>
        @endif
    </div>
    ...
    @if ($application)
        <div class="flex flex-wrap items-center justify-between gap-3"><flux:heading>{{ $application->application_code }}</flux:heading>
            @can('create', [App\Models\CandidateDocument::class, $application])
                <flux:button variant="primary" wire:click="create">{{ __('Tải tài liệu') }}</flux:button>
            @else
                <flux:badge>{{ __('Chỉ đọc: ứng dụng không thể chỉnh sửa') }}</flux:badge>
            @endcan
        </div>
        <div class="overflow-x-auto rounded-xl border border-zinc-200 p-4 dark:border-zinc-700">
            <flux:table :paginate="$records">
                <flux:table.columns><flux:table.column>{{ __('Tài liệu') }}</flux:table.column><flux:table.column>{{ __('Trạng thái đánh giá') }}</flux:table.column><flux:table.column>{{ __('Hành động') }}</flux:table.column></flux:table.columns>
                <flux:table.rows>
                    ...
                    @empty
                        <flux:table.row><flux:table.cell colspan="3"><div class="py-10 text-center">{{ __('Không có tài liệu cho hồ sơ này.') }}</div></flux:table.cell></flux:table.row>
                    @endforelse
                </flux:table.rows>
            </flux:table>
        </div>
    ...
    <flux:modal wire:model="showDeletion" class="md:w-96"><form wire:submit="delete" class="flex flex-col gap-5"><flux:heading>{{ __('Xóa tài liệu này hay không?') }}</flux:heading><flux:text>{{ __('Tệp và bản ghi sẽ bị xóa. Hành động này không thể hoàn tác.') }}</flux:text><flux:error name="deletion" /><div class="flex justify-end gap-3"><flux:modal.close><flux:button>{{ __('Hủy bỏ') }}</flux:button></flux:modal.close><flux:button type="submit" variant="danger" wire:loading.attr="disabled" wire:target="delete">{{ __('Xóa') }}</flux:button></div></form></flux:modal>
</section>
